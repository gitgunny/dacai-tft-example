...
function screen_screen_5.on_control_notify(screen, control, value)
    ...
    elseif control == notify_change_screen_disable_button_id then
        ...
        notify_change_screen_state = STATE_DISABLE
        notify_change_screen_count = 0
        set_text(screen_5_screen_id, notify_change_screen_count_text_id, "알림 활성화 후 스크린 전환 횟수: 0번")
    end
end
...
--- @param screen number 호출 스크린 ID
function screen_screen_5.on_screen_change(screen)
    if notify_change_screen_state == STATE_ENABLE then
        notify_change_screen_count = notify_change_screen_count + 1
        set_text(screen_5_screen_id, notify_change_screen_count_text_id, "알림 활성화 후 스크린 전환 횟수: " .. notify_change_screen_count .. "번")
    end
end

return screen_screen_5
