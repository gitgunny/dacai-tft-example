...
function screen_menu_child_3.on_control_notify(screen, control, value)
    ...
    elseif control == button_1_button_id then
        ...
        set_text(menu_2_screen_id, menu_value_text_id, "C-3A")
        child_screen_menu_close()
    elseif control == button_2_button_id then
        -- 2번째 버튼 터치 시 상위 스크린 메뉴 텍스트에 표시 후 나가기
        set_text(menu_2_screen_id, menu_value_text_id, "C-3B")
        child_screen_menu_close()
    elseif control == button_3_button_id then
        -- 3번째 버튼 터치 시 상위 스크린 메뉴 텍스트에 표시 후 나가기
        set_text(menu_2_screen_id, menu_value_text_id, "C-3C")
        child_screen_menu_close()
    elseif control == button_4_button_id then
        -- 4번째 버튼 터치 시 상위 스크린 메뉴 텍스트에 표시 후 나가기
        set_text(menu_2_screen_id, menu_value_text_id, "C-3D")
        child_screen_menu_close()
    end
end

return screen_menu_child_3
